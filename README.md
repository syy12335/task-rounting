...
Environment-Runtime 是一个面向工程场景的轻量级 Agent Runtime。随着 OpenClaw 在企业工程流程中的关注度持续提升，一个现实需求也逐渐显现：面向通用场景的 agent runtime 设计未必能直接覆盖所有工程落地形态。这个项目把视角收窄到高确定性工程流程，希望把常见工程任务组织成更清晰、更可复用、更容易维护的运行时。项目还在成长阶段，欢迎认领 issue、提交 PR，一起参与开发。
...
- **更低 token 消耗**：在当前业务样例测试中，整体 token 消耗约为直接使用 OpenClaw 的 7%；这个收益来自整套运行时的轻量化设计，包括显式状态视图、分层路由、workflow / skill 执行路径和异步回填机制。
- **更适合长任务异步执行**：长任务可以先返回 `running`，后续异步回填结果；同步回合只承载当前需要即时完成的部分。
- **更明确的状态约束**：controller 只根据显式 Environment 视图做决策，让路由行为更容易检查和复现。
...
很多通用 agentic 框架会优先保留开放场景下的灵活性；而在更窄的工程场景里，一部分任务的执行路径相对固定。README 里用测试类任务举例，只是为了把分层路由讲清楚，并不代表框架只服务测试场景。

Environment-Runtime 的做法是：把任务按确定性拆成多层执行路径，越确定的任务越早进入更稳定的 workflow / skill 路径。controller 只负责路由和结构化 task 生成，默认 `max_steps=3`；只有剩余高不确定性任务才进入完整 executor loop（默认 `max_steps=4`）。
...
└──────────────┬──────────────────────────────┬───────────┘
    ...
    │ 不阻塞当前轮         │     │ 灵活处理    │ 阻塞有返回值 │ 非阻塞派发
    └─────────────────────┘     └────────────────────────────┘
```

说明：图中的 `functest / accutest / perftest` 是当前仓库内置的示例 workflow type task，用于演示高确定性任务的分层路由路径；实际落地时可新增同格式 controller skill 包来接入你的业务 workflow。

核心直觉是：重试越多、每次 IO 带入的上下文越大，不同执行层的成本差异就越明显；能稳定沉淀为 workflow 的任务，可以交给更确定的路径处理，把完整 agentic loop 留给更需要灵活性的请求。

额外 LLM 参与度：

说明：这里描述的是 controller 完成路由之后，不同执行层新增的 LLM 参与程度；所有请求仍会先经过 controller。
...
| executor 灵活处理 | 完整参与 | 进入完整 executor agentic loop（默认 `max_steps=4`），适合高不确定性请求 |
...
# 也可以显式指定 conda 环境；不指定时会优先使用当前 CONDA_DEFAULT_ENV
