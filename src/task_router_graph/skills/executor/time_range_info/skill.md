...
- 若 `web_search` 返回结果明显与目标主题无关（如文学/影视/日历等噪声）且已尝试 2 次以内重写 query 后仍无有效结果：必须立即 `finish`，不得继续 observe 直到超步数。
- 若出现配额类错误（例如 `web_search quota exceeded`）：必须立即 `finish`，不得继续 observe。
