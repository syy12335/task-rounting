...
from task_router_graph_train.runtime_adapter import ASSETS_ROOT
...
def test_prepare_round_assets_builds_required_outputs(tmp_path: Path) -> None:
    ...
    assert (round_dir / "preference_admissions.jsonl").exists()
    ...
    assert report["counts_by_split"]["preference_admissions"] == 0
    assert (round_dir / "preference_admissions.jsonl").read_text(encoding="utf-8") == ""
    assert holdout_rows
    assert sft_rows
    assert grpo_rows
...
def test_prepare_round_assets_does_not_merge_previous_admissions(tmp_path: Path) -> None:
    round_root = tmp_path / "rounds"
    ...
    (first_dir / "sft_admissions.jsonl").write_text(
        '{"sample_id":"adm_001","state_input":{},"reference_action":{"action_kind":"observe","tool":"build_context_view","args":{"round_limit":3,"include_trace":false,"include_user_input":true,"include_task":true,"include_reply":true},"reason":"补充观察"},"reason":"admit","source_round":"round_0001"}\n',
        encoding="utf-8",
    )
    ...
    assert "adm_001" not in all_ids
    assert "adm_001" not in grpo_ids
    assert (second_dir / "preference_admissions.jsonl").read_text(encoding="utf-8") == ""
    assert second["counts_by_split"]["preference_admissions"] == 0
